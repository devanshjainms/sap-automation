...
validate_extra_parameters() {
	...
	if ! [[ "$value" =~ $qa_extra_parameters_pattern ]]; then
		exit_error "The $description must be a sequence of '-e key=value' arguments. Quoting, shell metacharacters and positional arguments such as playbook paths are not accepted." 2
	fi
}
...
for ((index = 0; index < ${#setup_source_tokens[@]}; index += 2)); do
	...
	sap_automation_qa_fork)
		if ! [[ "$parameter_value" =~ ^[A-Za-z0-9-]+$ ]]; then
			exit_error "The sap_automation_qa_fork value must be a GitHub owner name." 2
		fi
		setup_parameters="$setup_parameters -e $assignment"
		;;
	sap_automation_qa_version)
		if ! [[ "$parameter_value" =~ ^[A-Za-z0-9._/@+-]+$ ]]; then
			exit_error "The sap_automation_qa_version value must be a branch, tag, or commit." 2
		fi
		setup_parameters="$setup_parameters -e $assignment"
		;;
	esac
