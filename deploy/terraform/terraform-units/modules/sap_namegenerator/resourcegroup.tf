...
locals {
  ...
  backup_unit_name = upper(format("%s-%s-%s", local.landscape_env_verified, local.location_short, local.backup_vnet_verified))
  ...
  backup_tfstate_storageaccount_name              = substr(replace(lower(format("%s%sbackuptfstate%s", local.env_verified, local.location_short, local.random_id_verified)), "/[^a-z0-9]/", ""), 0, var.azlimits.stgaccnt)
