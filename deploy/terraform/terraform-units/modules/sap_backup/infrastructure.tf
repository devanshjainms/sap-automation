...
resource "azurerm_virtual_network" "vnet_backup" {
  ...
  location                = local.resource_group_exists ? (
                              data.azurerm_resource_group.backup[0].location) : (
                              azurerm_resource_group.backup[0].location
                            )
  resource_group_name     = local.resource_group_exists ? (
                              data.azurerm_resource_group.backup[0].name) : (
                              azurerm_resource_group.backup[0].name
                            )
...
data "azurerm_virtual_network" "vnet_backup" {
  ...
  name                    = split("/", var.infrastructure.vnets.backup.id)[8]
  resource_group_name     = split("/", var.infrastructure.vnets.backup.id)[4]
}
...
data "azurerm_subnet" "subnet_backup" {
  ...
  name                   = var.infrastructure.vnets.backup.subnet_backup.name
