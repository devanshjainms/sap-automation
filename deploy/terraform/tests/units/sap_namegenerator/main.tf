# Test wrapper for sap_namegenerator terraform-units module.
# Passes all variables through to the actual module and exposes outputs.
# This enables running tests from deploy/terraform/tests/ instead of inside the module directory.
...
module "naming" {
  ...
  library_environment        = var.library_environment
  deployer_environment       = var.deployer_environment
  landscape_environment      = var.landscape_environment
  location                   = var.location
  codename                   = var.codename
  management_vnet_name       = var.management_vnet_name
  sap_vnet_name              = var.sap_vnet_name
  sap_sid                    = var.sap_sid
  web_sid                    = var.web_sid
  db_sid                     = var.db_sid
  random_id                  = var.random_id
  db_ostype                  = var.db_ostype
  app_ostype                 = var.app_ostype
  db_platform                = var.db_platform
  anchor_ostype              = var.anchor_ostype
  app_server_count           = var.app_server_count
  scs_server_count           = var.scs_server_count
  web_server_count           = var.web_server_count
  db_server_count            = var.db_server_count
  iscsi_server_count         = var.iscsi_server_count
  ...
  sapautomation_name_limits  = var.sapautomation_name_limits
  azlimits                   = var.azlimits
  region_mapping             = var.region_mapping
  resource_prefixes          = var.resource_prefixes
  resource_suffixes          = var.resource_suffixes
  app_zones                  = var.app_zones
  scs_zones                  = var.scs_zones
  web_zones                  = var.web_zones
  db_zones                   = var.db_zones
  custom_prefix              = var.custom_prefix
  use_prefix                 = var.use_prefix
  deployer_location          = var.deployer_location
  utility_vm_count           = var.utility_vm_count
  scale_out                  = var.scale_out
}

output "naming" {
  value = module.naming.naming
}

output "naming_new" {
  value = module.naming.naming_new
}
