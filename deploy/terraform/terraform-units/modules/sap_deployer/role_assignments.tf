...
resource "azurerm_role_assignment" "role_assignment_msi" {
  provider                             = azurerm.main
  count                                = var.assign_subscription_permissions && var.key_vault.enable_rbac_authorization && length(var.deployer.user_assigned_identity_id) == 0 ? 1 : 0
  scope                                = var.key_vault.exists ? data.azurerm_key_vault.kv_user[0].id : azurerm_key_vault.kv_user[0].id
...
resource "azurerm_role_assignment" "role_assignment_additional_users" {
  provider                             = azurerm.main
  count                                = var.assign_subscription_permissions && var.key_vault.enable_rbac_authorization && !var.key_vault.exists && length(compact(var.additional_users_to_add_to_keyvault_policies)) > 0 ? (
...
resource "azurerm_role_assignment" "role_assignment_webapp" {
  provider                             = azurerm.main
  count                                = var.assign_subscription_permissions && var.key_vault.enable_rbac_authorization && !var.key_vault.exists  && var.app_service.use ? 1 : 0
  scope                                = var.key_vault.exists ? data.azurerm_key_vault.kv_user[0].id : azurerm_key_vault.kv_user[0].id
